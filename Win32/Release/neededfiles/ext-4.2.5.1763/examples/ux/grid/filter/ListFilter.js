...
Ext.define('Ext.ux.grid.filter.ListFilter', {
    ...
    init : function (config) {
        ...
        me.dt = Ext.create('Ext.util.DelayedTask', me.fireUpdate, me);

        // If the List filter is auto-creating its store from the unique values in the grid store (i.e., no `options` or `store`
        // configs), it will need to listen to grid store events to properly sync its options when the grid store changes.
        if (!me.options && !me.store) {
            menu = me.menu;
            ...
            me.grid.store.on({
                scope: me,
                datachanged: menu.onDataChanged,
                update: menu.onDataChanged
            });
        }
    },
